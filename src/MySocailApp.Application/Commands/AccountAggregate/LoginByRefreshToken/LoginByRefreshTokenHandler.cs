...
{
    public class LoginByRefreshTokenHandler(UserManager<Account> userManager, ITokenService tokenService, IMapper mapper) : IRequestHandler<LoginByRefreshTokenDto, LoginResponseDto>
    {
        private readonly UserManager<Account> _userManager = userManager;
        private readonly ITokenService _tokenService = tokenService;
        private readonly IMapper _mapper = mapper;

        public async Task<LoginResponseDto> Handle(LoginByRefreshTokenDto request, CancellationToken cancellationToken)
        {
            var account =
                await _userManager.Users.FirstAsync(x => x.Id == request.Id && !x.IsRemoved) ??
