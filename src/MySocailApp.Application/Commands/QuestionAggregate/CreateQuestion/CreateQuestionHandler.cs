...
{
    public class CreateQuestionHandler(QuestionManager manager, IAccessTokenReader accessTokenReader, IUnitOfWork unitOfWork, IQuestionWriteRepository repository, IQuestionReadRepository readRepository, IMapper mapper) : IRequestHandler<CreateQuestionDto, QuestionResponseDto>
    {
        ...
        public async Task<QuestionResponseDto> Handle(CreateQuestionDto request, CancellationToken cancellationToken)
        {
            ...
            return _mapper.Map<QuestionResponseDto>(
                await _readRepository.GetByIdAsync(question.Id, cancellationToken)
            );
        }
    }
}
