...
AS BEGIN
	...
	SELECT TOP(@take) 
		[all].[Id],[accounts].UserName_Value as UserName, [all].[ConversationId], [all].[Content], [all].[CreatedAt], [all].[IsEdited], 
		[all].[UpdatedAt], [all].[SenderId], [all].[ReceiverId], [all].[State],[all].[NumberOfMedias], [all].[IsOwner]
	FROM
	(
		SELECT
			[pM].[Id], [pM].[ConversationId], [pM].[Content], [pM].[CreatedAt], [pM].[IsEdited], [pM].[UpdatedAt], [pM].[SenderId],[pm].[NumberOfMedias], [pM].[ReceiverId], [pM].[row],
