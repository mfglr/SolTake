...
namespace MySocailApp.Infrastructure.CommentAggregate
{
    public class CommentWriteRepository(AppDbContext context) : ICommentWriteRepository
    {
        ...
        {
            var likes = await _context.CommentUserLikes.Where(x => x.UserId == userId).ToListAsync(cancellationToken);
        ...
        {
            var notifications = await _context.CommentUserLikeNotifications.Where(x => x.UserId == userId).ToListAsync(cancellationToken);
