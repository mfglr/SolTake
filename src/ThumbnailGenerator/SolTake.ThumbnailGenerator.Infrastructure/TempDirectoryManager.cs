...
{
    internal class TempDirectoryManager : ITempDirectoryManager
    {
        ...
        public Task<Stream> ReadAsync(string blobName, CancellationToken cancellationToken)
        {
            var path = _pathFinder.GetPath(ScopeContainerName, blobName);
            if(!File.Exists(path))
                throw new Exception("Blob not found");
            return Task.FromResult(File.OpenRead(path) as Stream);
        }
    }
}
