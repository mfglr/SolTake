...
extension MapExtentions<K extends Comparable, V extends Entity<K>> on Map<K,V>{
  ...
  Map<K,V> updateMany(Iterable<V> values) =>
    { for (var e in [...this.values.map((e) => values.where((x) => x.id.compareTo(e.id) == 0).firstOrNull ?? e)]) e.id : e };
  ...
  Map<K,V> setOne(V value) => { for (var e in [...values, value]) e.id : e };
  Map<K,V> setMany(Iterable<V> values) => { for (var e in [...this.values, ...values]) e.id : e };
  Map<K,V> removeOne(K id) => { for (var e in [...values.where((e) => e.id.compareTo(id) != 0)]) e.id : e };
}
