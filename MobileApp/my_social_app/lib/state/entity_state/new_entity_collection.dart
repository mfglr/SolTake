...
class NewEntityCollection<K extends Comparable, V extends Entity<K>> {
  ...
  const NewEntityCollection._({required Map<K, NewEntityContainer<K, V>> map}) : _map = map;

  NewEntityCollection<K,V> loading(K key) =>
    NewEntityCollection<K,V>._(
      map: _map.setOne(key, NewEntityContainer.loading(key))
    );
  NewEntityCollection<K,V> success(V entity) =>
    NewEntityCollection<K,V>._(
      map: _map.setOne(entity.id, _map[entity.id]?.success(entity))
    );
  NewEntityCollection<K,V> failed(K key) =>
    NewEntityCollection<K,V>._(
      map: _map.setOne(key, _map[key]?.failed())
    );
  NewEntityCollection<K,V> notFound(K key) =>
    NewEntityCollection<K,V>._(
      map: _map.setOne(key, _map[key]?.notFound())
    );
  NewEntityCollection<K,V> removeOne(V entity) =>
    NewEntityCollection<K,V>._(
      map: _map.removeOne(entity.id)
    );
  ...
  NewEntityCollection<K,V> successOne(V entity) =>
    NewEntityCollection<K,V>._(
      map: _map.successOne(entity)
    );
  NewEntityCollection<K,V> successMany(Iterable<V> entities) =>
    NewEntityCollection<K,V>._(
      map: _map.successMany(entities)
    );
