...
void getNextPageUserQuestionsIfNoPageMiddleware(Store<AppState> store,action,NextDispatcher next){
  if(action is GetNextPageUserQuestionsIfNoPageAction){
    final pagination = store.state.userEntityState.entities[action.userId]!.questionPaginations.paginations[action.offset]!;
    if(pagination.isReadyForNextPage && !pagination.hasAtLeastOnePage){
      store.dispatch(GetNextPageUserQuestionsAction(offset: action.offset, userId: action.userId));
    }
  }
  next(action);
}
...
void nextPageOfUserQuestionsMiddleware(Store<AppState> store,action,NextDispatcher next){
  if(action is GetNextPageUserQuestionsAction){
    final paginations = store.state.userEntityState.entities[action.userId]!.questionPaginations; 
    final offset = paginations.getNextOffset(action.offset);
    QuestionService()
      .getByUserId(action.userId,offset,questionsPerPage,true)
      .then((questions){
        store.dispatch(AddNextPageUserQuestionsAction(offset: action.offset, userId: action.userId,questionIds: questions.map((e) => e.id)));
...
void addUserQuestionsPaginationMiddleware(Store<AppState> store,action,NextDispatcher next){
  if(action is AddUserQuestionsPaginationAction){
    if(store.state.userEntityState.entities[action.userId] == null){
      UserService()
        .getById(action.userId)
        .then((user){
          store.dispatch(AddUserAction(user: user.toUserState()));
          store.dispatch(AddUserImageAction(image: UserImageState.init(user.id)));
          store.dispatch(AddUserQuestionsPaginationSuccessAction(offset: action.offset, userId: action.userId));
        });
    }
    else{
      store.dispatch(AddUserQuestionsPaginationSuccessAction(offset: action.offset, userId: action.userId));
