...
class _TakeMediaPageState extends State<TakeMediaPage> {
  ...
  Future<void> _takePhoto() async {
    ...
    var dimention = await DimentionCalculator.calculate(image.path);
    if(mounted){
      Navigator
        .of(context)
        .pop<LocalImage>(LocalImage(file: File(image.path), dimention: dimention));
    }
  ...
  Future<void> _stopVideoRecording() async {
    ...
    var dimention = await DimentionCalculator.calculate(video.path);
    if(mounted){
      Navigator
        .of(context)
        .pop<LocalVideo>(LocalVideo(file: File(video.path), dimention: dimention));
    }
  }
  ...
  void _startTimer(){
    Timer.periodic(
      ...
      (timer) async {
        if(timer.tick >= widget.maxDurationInSeconds && mounted){
          timer.cancel();
